...
public:
  uintt():v(0){}
  uintt(const unsigned& a):v(a){}
  operator unsigned()const { return v; }
  friend
  ostream& operator<<(ostream& out, const uintt& r){ return out << r.v; }
  friend
  istream& operator>>(istream& in,  uintt& r){ return in >> r.v; }
};

using naturalne = uintt;
...
ostream& operator<<(ostream& out, const blad& p){
  return out << p.txt << " || Blad w linijce: " << p.lp << endl;
}
//---===*****===---
...
void polygon::read_check(ifstream& f){
  cout << "Wczytywanie danych z pliku oraz sprawdzanie... " << endl;
  string buf;
  getline(f, buf);
  if(buf != "[POLYGON]") throw string("Brak naglowka \"[POLYGON]\"");
  cout << buf << endl;
  ...
  for(unsigned i = 0; i < n; i++){
    ...
    double x, y;
    f >> x >> y;
    if(!f) throw blad("Blad przy wczytawaniu danych", i + 1);
    points[num - 1] = point(x, y);
    cout << num << ": " << points[num - 1] << endl;
  }
...
double polygon::pole()const{
  ...
  if(pole <= 0) {
    throw blad("Pole jest ujemne, zly ksztalt figury", 1);
  }
  return 0.5 * abs(pole);
}
