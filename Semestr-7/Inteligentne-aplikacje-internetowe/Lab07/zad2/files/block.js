...
function insertValues(svg, val, log) {
  ...
  for (let v of val) {
    ...
    if (log && pos > 0.0) pos = Math.log10(1.0 + pos * 9.0);
    let posx = 26 + 360 * pos;
    let posy = 140;
...
function insertInterface(svg, val, bez, log) {
  ...
  if (bez.length != 4) {
    console.log(
      "selected attribute need 4 numeric values separated by commas",
      bez
    );
    return;
  }
  ...
  svg.onmousemove = (event) => {
    ...
    if (move.next != null && px >= parseInt(move.next.getAttribute("cx")))
      px = parseInt(move.next.getAttribute("cx")) - 1;
    if (move.prev != null && px <= parseInt(move.prev.getAttribute("cx")))
      px = parseInt(move.prev.getAttribute("cx")) + 1;
  ...
  svg.getvalues = () => {
    ...
    return (
      "0," +
      svg.parentNode.getAttribute("id") +
      "," +
      min +
      "," +
      max +
