...
func GetBeersFromGroup(grupa int, limit int) []*Beer {
	return queryBeers("SELECT b.* FROM beers b LEFT JOIN grpa g ON (b.id = g.id) WHERE g.grupa = ? ORDER BY RAND() LIMIT ?;", grupa, limit)
}

func GetBeersFromGroupB(grupa int, limit int) []*Beer {
	// analogicznie pobranie piw z tabeli grpb (grupa utworzona dla r = 20)
	return queryBeers("SELECT b.* FROM beers b LEFT JOIN grpb g ON (b.id = g.id) WHERE g.grupa = ? ORDER BY RAND() LIMIT ?;", grupa, limit)
}
